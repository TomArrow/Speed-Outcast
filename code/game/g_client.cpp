...
void G_SetG2PlayerModel( gentity_t * const ent, const char *modelName, const char *customSkin, const char *surfOff, const char *surfOn )
{
	...
	ent->s.modelindex2 = G_SkinIndex(skinName);
	ent->playerModel = gi.G2API_InitGhoul2Model( ent->ghoul2, va("models/players/%s/model.glm", modelName), ent->s.modelindex, ent->s.modelindex2, NULL, 0, 0 );
	if (ent->playerModel == -1)
	{//try the stormtrooper as a default
		modelName = "stormtrooper";
		ent->s.modelindex = G_ModelIndex(va("models/players/%s/model.glm", modelName));
		//ent->s.modelindex2 = G_SkinIndex("default");
